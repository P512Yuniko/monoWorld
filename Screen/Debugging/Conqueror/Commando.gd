...
var succinct = 0
func contigency():
	if (Key.armor or $Weapon/Inventory/Equipment/Kit/Armor.is_hovered() and Key.fired) and Inventory.armor > 0 and armor < 150 and !armor_using and abs(velocity.x) < 1600 and !$Weapon.throwing and !$Weapon.switching and !$Weapon.mode_changing and !$Weapon.reloading and !$Weapon.fire_delay and !$Weapon.aim and !slash and !climb and !$Weapon/Heal.time_left > 0:
...
func melee():
	...
	if Key.attack or Key.attacked and $Weapon.echo > 0:
		slash_buffer = 8
	if slash_buffer > 0:
...
func movement():
	...
	if (Key.jump or jump_buffer > 0) and (count_jump > 0 or climb) and !($Ouch.is_colliding() and $Grounded.is_colliding()) and !slash_delay and !(sliding and slash) and !($Blub.is_colliding() and !($Grounded.is_colliding() and !$Breath.is_colliding())):
		normal_jump()
		dashing = false
		$DashTimer.stop()
		if $Grounded.is_colliding():
	...
	if !$DashCool.time_left > 0:
		if Key.left:
			...
			if dash_timer_right < 10 and Setting.dash_mode < 3:
				dash_timer_right = 20
			dash_timer_left = 0
		if !$Weapon.aim and !$Weapon.fire_delay and !slash and !Key.up and !Key.down and !$BlockL.is_colliding() and !$Ouch.is_colliding():
			...
			if Key.left and dash_timer_left > 0 and dash_timer_left < 18 and Setting.dash_mode < 2 or Key.dash and !facing_right and Setting.sprint_mode < 2:
				...
				dash_cancel = 10
		if !$Weapon.aim and !$Weapon.fire_delay and !slash and !Key.up and !Key.down and !$BlockR.is_colliding() and !$Ouch.is_colliding():
	...
	if !slash:
		...
		if $Weapon.bare and crouching or $Weapon.throwing and !$Weapon/Sentry.time_left > 0 or $Weapon/Armor.time_left > 0.1 or $Weapon/Heal.time_left > 0.1:
			...
			if (($SpriteR/Base/TayL.global_position.x < $SpriteR/Skeleton/Body/UpperArmL.global_position.x + 10 and facing_right) or ($SpriteL/Base/TayL.global_position.x > $SpriteL/Skeleton/Body/UpperArmR.global_position.x - 10 and !facing_right)):
				...
				$AniArmRotateL.play("Up")
	if slash or $Weapon.fire_delay or $Weapon.reloading or $Weapon.switching or $Weapon.mode_changing or $Weapon.block or $Weapon.aim or abs(velocity.x) > 200 or abs(velocity.y) > 200 or Key.up or Key.down or $Hurt.time_left > 0 or hp < 20:
		wait_count = 120
	else:
