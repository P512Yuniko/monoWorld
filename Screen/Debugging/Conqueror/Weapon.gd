...
var on_slot_1 = true
func switch():
	if (Key.scroll_up and !pack_opening or $Inventory/Weapon/Weapon1.is_hovered() and Input.is_action_just_pressed("ui_select")) and !melee:
		if switch_count > 0:
			if switch_count > 1:
				switch_count -= 1
				on_slot_1 = true
			...
			switch_count += 1
			$Switch.start(0.5)
	if (Key.scroll_down and !pack_opening or $Inventory/Weapon/Weapon2.is_hovered() and Input.is_action_just_pressed("ui_select")) and !melee:
		if switch_count > 0:
			...
			switch_count += 1
			$Switch.start(0.5)
	if Key.scroll and !melee and get_child(1).get_class() == "Node2D":
		if switch_count > 0:
			on_slot_1 = !on_slot_1
			$Switch.start(0.5)
		else:
...
func weapon():
	...
	if $Switch.time_left > $Switch.wait_time - 0.1 or $Lethal.time_left < 0.1 and $Lethal.time_left > 0 or $Tactical.time_left < 0.1 and $Tactical.time_left > 0 or $Sentry.time_left < 0.1 and $Sentry.time_left > 0 or $Chaser.time_left < 0.1 and $Chaser.time_left > 0 or $Armor.time_left < 0.1 and $Armor.time_left > 0 or $Heal.time_left < 0.1 and $Heal.time_left > 0:
		...
		if !lethal_hold and !$Lethal.time_left > 0 and !$Sentry.time_left > 0:
			...
			$Rotation/Recoil/Body/ArmR/HandR.global_position = gun.get_node("Body/HandR/HandR2").global_position
		if !throwing and !$Armor.time_left > 0 and !$Heal.time_left > 0 and !Key.map:
...
func throw():
	...
	if tactical_hold:
		if hold < 60 and !$Tactical.time_left > 0.2:
			hold += 1 * Setting.inverse
	if throwing or $Armor.time_left > 0.1 or $Heal.time_left > 0.1 or Key.map:
