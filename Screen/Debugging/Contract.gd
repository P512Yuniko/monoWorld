...
var contract = 0
var vision = false
@onready var bullet = preload("res://Screen/Asset/Entity/Bullet/Enemy/Ngu/Nuoc.tscn")
...
func _physics_process(_delta):
	...
	if hp >= 1:
		...
		if !killed:
			...
			if contract:
				Contract.progress[Contract.contract.find(0)] -= 1
				Contract.update()
			killed = true
...
func _on_fire_timeout():
	...
	bullet_ins.scale.x =  $Body.scale.x
	bullet_ins.dmg = dmg
	bullet_ins.apply_impulse(Vector2(2000 * $Body.scale.x, 0).rotated(0), Vector2())
