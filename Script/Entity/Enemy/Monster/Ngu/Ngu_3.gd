...
var killer = 0
var focus = 0
@onready var bullet = preload("res://Screen/Asset/Entity/Bullet/Enemy/Ngu/Banhmi.tscn")
...
func active():
	...
	if focus > 25 and !$Fire.time_left > 0:
		for i in 2:
			...
			bullet_ins.angular_velocity = 5 * $Body.scale.x
			bullet_ins.dmg = dmg
			bullet_ins.apply_impulse(Vector2(randf_range(200,800) * $Body.scale.x, -2000).rotated(0), Vector2())
