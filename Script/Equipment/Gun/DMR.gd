...
func _physics_process(_delta):
	...
	if switching or block or action:
		...
		$Empty.stop()
		$Empty2.stop()
	if $Firedelay.time_left > fire_rate * 3/4:
...
func attach():
	...
	if muzzle == 0 or muzzle > 3:
		$Flash.show()
		$Body/Muzzle.hide()
		$Flash.position.x = 195
		$Gas.position.x = 195
		$Body/Smoke.position.x = 105
	else:
		$Body/Muzzle.show()
		$Flash.position.x = 205
...
func magout():
	...
	mag_ins.angular_velocity = -40 * facing
	mag_ins.facing = facing
	mag_ins.apply_impulse(Vector2(randf_range(200,400), randf_range(-100, -300) * facing).rotated(global_rotation), Vector2())
