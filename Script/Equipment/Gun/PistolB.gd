...
@export var base_recoil = 0.5
@export var base_spread = 2.0
@export var base_ammo = 7
var reverse_ammo = 0
@export var empty_time = 1.5
@export var tac_time = 1.4
var type = 0
var scope = 0
var foregrip = 0
var muzzle = 0
...
func _physics_process(_delta):
	if $Firedelay.time_left > 0:
		...
		fire_delay = false
	if $Firedelay.time_left < fire_rate/2 and buffering > 0 and can_fire and current_ammo > 0:
		$Firedelay.stop()
		fire_delay = false
	if $Firedelay.time_left > fire_rate * 2/3:
		coiling = true
	else:
		coiling = false
	if smoke >= 0 :
	...
	if $Firedelay.time_left > 0:
		...
		if walk:
			...
			$Anim.play("Idle")
	scale.y = facing
	$Body/Body/Counter.text = str(current_ammo)
	$Body/Body/Counter.scale.x = facing
	if facing == 1:
		$Body/Body/Upper.frame = 2
	else:
		$Body/Body/Upper.frame = 1
	reverse_ammo = ammo[1]
...
func attach():
	...
	if scope == 0 or scope > 2:
		$Body/Body/Scope.hide()
	else:
		$Body/Body/Scope.show()
		if scope == 1:
			$Body/Body/Scope.frame = 0
			aim_speed += 0.05
		elif scope == 2:
			$Body/Body/Scope.frame = 1
	if muzzle == 0 or muzzle > 3:
		$Flash.show()
		$Body/Body/Muzzle.hide()
		$Flash.position.x = 134
		$Body/Smoke.position.x = 50
		$Gas.position.x = 130
	else:
		$Body/Body/Muzzle.show()
		$Flash.position.x = 147
		$Body/Smoke.position.x = 70
		if muzzle == 1:
			$Flash.hide()
			$Body/Body/Muzzle.frame = 12
			$Gas.position.x = 175
		elif muzzle == 2:
			$Flash.show()
			$Body/Body/Muzzle.frame = 13
			$Gas.position.x = 145
			recoil -= base_recoil/4
		elif muzzle == 3:
			$Flash.show()
			$Body/Body/Muzzle.frame = 14
			$Gas.position.x = 150
			max_spread -= base_spread/4
	if laser == 0 or laser > 1:
		$Body/Body/Laser.hide()
	else:
		$Body/Body/Laser.show()
		max_spread -= base_spread/4
		aim_speed += 0.05
	if magazine > 2:
		type = magazine - 2
...
func magout():
	...
	mag_ins.position = $Body/Body/MagOut.get_global_position()
	mag_ins.angular_velocity = 5 * facing
	mag_ins.facing = facing
	mag_ins.apply_impulse(Vector2(randf_range(400,800), 0).rotated(global_rotation), Vector2())
	get_tree().get_root().add_child(mag_ins)
func magout2():
	var mag_ins = mag.instantiate()
	mag_ins.position = $Body/Body/MagOut.get_global_position()
	mag_ins.facing = facing
	mag_ins.apply_impulse(Vector2(randf_range(-10,-50), 0).rotated(global_rotation), Vector2())
	get_tree().get_root().add_child(mag_ins)
