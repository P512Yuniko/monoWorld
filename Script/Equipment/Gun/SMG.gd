...
func attach():
	...
	if muzzle == 0 or muzzle > 3:
		...
		$Body/Smoke.position.x = 35
		$Gas.position.x = 115
	else:
		$Body/Muzzle.show()
		$Flash.position.x = 135
		$Body/Smoke.position.x = 60
		if muzzle == 1:
			$Flash.hide()
			$Body/Muzzle.frame = 8
			$Gas.position.x = 160
	...
	if magazine == 0 or magazine > 1:
		...
		$Mag3.frame = 5
		max_ammo = base_ammo
		mag = load("res://Screen/Asset/Entity/Mag/Mag45.tscn")
	else:
		$Body/Mag.frame = 7
		$Mag2.frame = 7
		$Mag3.frame = 7
		max_ammo = base_ammo + 10
		mag = load("res://Screen/Asset/Entity/Mag/Mag45B.tscn")
	if magazine > 2:
		type = magazine - 2
		recoil += base_recoil/2
		max_spread += base_spread/2
...
func magout():
	...
	mag_ins.apply_impulse(Vector2(randf_range(200,150), 0).rotated($Body/MagOut.global_rotation), Vector2())
