...
var reverse_ammo = 0
@export var base_ammo = 40
@export var empty_time = 1.6
@export var tac_time = 1.5
@export var mag_facing = 0
var type = 0
var scope = 0
var foregrip = 0
var muzzle = 0
...
func attach():
	...
	if muzzle == 0 or muzzle > 3:
		$Body/Muzzle.hide()
		$Flash.show()
		$Flash.position.x = 115
		$Body/Smoke.position.x = 45
		$Gas.position.x = 115
	else:
		$Body/Muzzle.show()
		$Flash.position.x = 135
		$Body/Smoke.position.x = 55
		if muzzle == 1:
			$Flash.hide()
			$Body/Muzzle.frame = 8
			$Gas.position.x = 160
	...
	if magazine == 0 or magazine > 1:
		...
		$Mag3.frame = 5
		max_ammo = base_ammo
		mag = preload("res://Screen/Asset/Entity/Mag/Mag7.tscn")
	else:
		$Body/Mag.frame = 3
		$Mag2.frame = 3
		$Mag3.frame = 3
		max_ammo = base_ammo + 10
		mag = preload("res://Screen/Asset/Entity/Mag/Mag7B.tscn")
	if magazine > 2:
		type = magazine - 2
		recoil += base_recoil/2
		max_spread += base_spread/2
...
func magout():
	...
	mag_ins.apply_impulse(Vector2(0,0).rotated($Body/MagOut.global_rotation), Vector2())
